...
public class DiamondChestScreenHandler extends ScreenHandler {
    ...
    public DiamondChestScreenHandler(int syncId, PlayerInventory playerInventory) {
        this(syncId, playerInventory, new SimpleInventory(88));
    }
    ...
    public DiamondChestScreenHandler(int syncId, PlayerInventory playerInventory, Inventory inventory) {
        super(SimpleProgressions.DIAMOND_CHEST_SCREEN_HANDLER, syncId);
        checkSize(inventory, 88);
        ...
        for (m = 0; m < 8; ++m) {
            for (l = 0; l < 11; ++l) {
                this.addSlot(new Slot(inventory, l + m * 11, l * 18 + 8, m * 18 + 14));
            }
        }
        //The player inventory
        for (m = 0; m < 3; ++m) {
            for (l = 0; l < 9; ++l) {
                this.addSlot(new Slot(playerInventory, l + m * 9 + 9, 26 + l * 18, 170 + m * 18));
            }
        }
        //The player Hotbar
        for (m = 0; m < 9; ++m) {
            this.addSlot(new Slot(playerInventory, m, 26 + m * 18, 228));
        }
