...
public class GoldChestBlock extends BlockWithEntity {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (world.isClient) {
            ...
            if (blockEntity instanceof GoldChestEntity) {
                player.openHandledScreen((GoldChestEntity)blockEntity);
                player.incrementStat(SimpleProgressions.OPEN_GOLD_CHEST);
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (blockEntity instanceof GoldChestEntity) {
            ((GoldChestEntity)blockEntity).tick();
        }
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        if (itemStack.hasCustomName()) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof GoldChestEntity) {
                ((GoldChestEntity)blockEntity).setCustomName(itemStack.getName());
            }
        }
