...
public class NetheriteChestBlock extends BlockWithEntity {
    public static final DirectionProperty FACING;
    public static final BooleanProperty OPEN;

    public NetheriteChestBlock(Settings settings) {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (world.isClient) {
            ...
            if (blockEntity instanceof NetheriteChestEntity) {
                player.openHandledScreen((NetheriteChestEntity)blockEntity);
                player.incrementStat(SimpleProgressions.OPEN_NETHERITE_CHEST);
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (blockEntity instanceof NetheriteChestEntity) {
            ((NetheriteChestEntity)blockEntity).tick();
        }
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        if (itemStack.hasCustomName()) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof NetheriteChestEntity) {
                ((NetheriteChestEntity)blockEntity).setCustomName(itemStack.getName());
            }
        }
